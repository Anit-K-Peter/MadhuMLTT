...
console.log('====================================================');
console.log('MADHU ML TT — PERFORMANCE BENCHMARK SUITE');
console.log('====================================================');
...
for (const { name, bytes } of sizes) {
  ...
  const isMatch = sample === unicode;
  if (!isMatch) allPassed = false;

  console.log(`[${name.padEnd(6)}] Payload: ${(actualBytes/1024).toFixed(1)} KB | Forward: ${forwardMs.padStart(7)} ms | Reverse: ${reverseMs.padStart(7)} ms | Output Fidelity: ${isMatch}`);
}

console.log('----------------------------------------------------');
console.log(`Benchmark Execution Complete: ${allPassed ? 'ALL PASSED' : 'FAILED'}`);
console.log('====================================================');
